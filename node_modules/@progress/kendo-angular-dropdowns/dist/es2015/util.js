 ...
 * @hidden
 */
export const resolveValuesInArray = (values, data = [], valueField) => data.filter(curr => {
    return values.some(item => item === curr[valueField]);
});
/**
 * @hidden
 */
...
export const isObjectArray = (values) => {
    return isArray(values) && values.some(item => isObject(item));
};
/**
 * @hidden
 */
export const selectedIndices = (values, data, valueField) => {
    ...
    return values.reduce((arr, item) => {
        const value = isPresent(item[valueField]) ? item[valueField] : item;
...
export const resolveValue = (args) => {
    ...
    if (isPresent(args.value)) {
        dataItem = args.data.find(element => getter(element, args.valueField) === args.value);
        return {
            dataItem: dataItem,
