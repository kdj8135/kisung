...
export class HeaderComponent {
    ...
    toggleSort(column) {
        const { allowUnsort, mode } = normalize(this.sortable, column.sortable);
    ...
    shouldActivate(column) {
        ...
        if (!canReorder && !isPresent(column.field)) {
            return false;
        }
        return this.groupable || canReorder;
    }
    isSortable(column) {
    ...
    toggleDirection(field, allowUnsort) {
    ...
    trackMove(leaveStream, dropStream, e) {
        ...
        if (changed && isFirst) {
            return e.draggable.drag
                ...
                .map(({ mouseEvent }) => mergeObjects({ changeContainer: true }, e, { mouseEvent }));
        }
        return of(e);
    }
