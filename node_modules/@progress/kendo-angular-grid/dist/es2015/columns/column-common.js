 ...
 * @hidden
 */
export const expandColumns = (columns) => (columns.reduce((acc, column) => acc.concat(isSpanColumnComponent(column) ? column.childColumns.toArray() : [column]), []) // tslint:disable-line:align
);
/**
 * @hidden
 */
 ...
 * @hidden
 */
export const columnsToRender = (columns) => (expandColumns(columns).filter(x => !x.hidden));
/**
 * @hidden
 */
export const columnsSpan = (columns) => (columns || []).reduce((acc, col) => acc + col.colspan, 0);
...
export const leafColumns = columns => {
    return columns.reduce((acc, column) => {
        ...
        return acc;
    }, []).filter(x => !x.hidden); // tslint:disable-line:align
};
/**
 * @hidden
 */
